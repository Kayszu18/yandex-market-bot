...
async def process_withdraw_amount(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        try:
            from database import get_admin_list
            admin_ids = get_admin_list()
            for admin_id in admin_ids:
                try:
                    await message.bot.send_message(
                        admin_id,
                        f"🔔 Yangi pul yechib olish so'rovi!\n\n"
                        f"🆔 So'rov raqami: #{request_id}\n"
                        f"👤 Foydalanuvchi: @{message.from_user.username or 'N/A'} (ID: {message.from_user.id})\n"
                        f"💰 Miqdor: {amount} so'm\n"
                        f"💳 To'lov usuli: {withdraw_type}\n"
                        f"📝 To'lov manzili: {wallet}\n\n"
                        f"Admin panel orqali tekshiring!"
                    )
                except Exception as e:
                    logger.error(f"Failed to notify admin {admin_id} about withdraw request: {e}")
        except Exception as e:
            logger.error(f"Failed to notify admins about withdraw request: {e}")
...
def register_handlers(dp):
    ...
    # Callback handlers
    dp.register_callback_query_handler(cancel_order_callback, lambda c: c.data.startswith("cancel_order_"), IsPrivateCallback(), IsNotBlockedCallback(), state="*")
    dp.register_callback_query_handler(back_to_main_menu, lambda c: c.data == "back_to_main", IsPrivateCallback(), IsNotBlockedCallback(), state="*")
